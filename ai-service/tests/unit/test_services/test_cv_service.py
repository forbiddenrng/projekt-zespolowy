...
class TestCVService:
  ...
  @pytest.fixture
  def cv_service(self, tmp_path):
    """Create CVService instance with temporary storage path"""
    with patch.dict(os.environ, {"CV_STORAGE_PATH": str(tmp_path / "cvs")}):
      service = CVService()
      yield service
  ...
  def sample_html(self):
    ...
    # ===== generate_cv_html tests =====
    
  def test_generate_cv_html_returns_string(self, cv_service, sample_cv_data):
    """Test that generate_cv_html returns a string"""
    with patch('app.services.cv_service.Environment') as mock_env_class:
      mock_template = Mock()
      ...
      result = cv_service.generate_cv_html(sample_cv_data)

      assert isinstance(result, str)
      assert result == "<html>CV</html>"

  def test_generate_cv_html_calls_template_render_with_cv_data(self, cv_service, sample_cv_data):
    ...
    with patch('app.services.cv_service.Environment') as mock_env_class:
      ...
      cv_service.generate_cv_html(sample_cv_data)

      mock_template.render.assert_called_once_with(sample_cv_data)

  def test_generate_cv_html_uses_correct_template_path(self, cv_service, sample_cv_data):
    """Test that FileSystemLoader is initialized with correct path"""
    with patch('app.services.cv_service.FileSystemLoader') as mock_loader_class:
      with patch('app.services.cv_service.Environment') as mock_env_class:
        mock_template = Mock()
        mock_template.render.return_value = "<html>CV</html>"
        mock_env = Mock()
  ...
  def test_generate_cv_html_loads_template_file(self, cv_service, sample_cv_data):
    ...
    with patch('app.services.cv_service.Environment') as mock_env_class:
      ...
      mock_env.get_template.assert_called_once_with("template.html")

  def test_generate_cv_html_with_empty_data(self, cv_service):
    """Test generate_cv_html with empty CV data"""
    with patch('app.services.cv_service.Environment') as mock_env_class:
      mock_template = Mock()
      ...
      result = cv_service.generate_cv_html({})
  ...
  @pytest.mark.asyncio
  async def test_save_locally_creates_directory_structure(self, cv_service, sample_html):
    """Test that _save_locally creates proper directory structure YYYY/MM/user_id"""
    user_id = "test_user_123"
    task_id = "task_456"

    with patch('app.services.cv_service.HTML') as mock_html_class:
      mock_html_instance = Mock()
      mock_html_class.return_value = mock_html_instance

      result = await cv_service._save_locally(user_id, task_id, sample_html)
  ...
  @pytest.mark.asyncio
  async def test_save_locally_calls_html_write_pdf(self, cv_service, sample_html):
    ...
    with patch('app.services.cv_service.HTML') as mock_html_class:
      ...
      await cv_service._save_locally(user_id, task_id, sample_html)

      mock_html_class.assert_called_once_with(string=sample_html)
      mock_html_instance.write_pdf.assert_called_once()

  @pytest.mark.asyncio
  async def test_save_locally_uses_utc_timestamp(self, cv_service, sample_html):
    """Test that _save_locally uses UTC timezone for date"""
    user_id = "test_user"
    task_id = "test_task"

    with patch('app.services.cv_service.HTML') as mock_html_class:
      with patch('app.services.cv_service.datetime') as mock_datetime:
  ...
  @pytest.mark.asyncio
  async def test_save_locally_returns_string(self, cv_service, sample_html):
    """Test that _save_locally returns a string path"""
    with patch('app.services.cv_service.HTML') as mock_html_class:
      mock_html_instance = Mock()
      mock_html_class.return_value = mock_html_instance

      result = await cv_service._save_locally("user", "task", sample_html)
  ...
  @pytest.mark.asyncio
  async def test_save_pdf_delegates_to_save_locally(self, cv_service, sample_html):
    """Test that save_pdf delegates to _save_locally"""
    user_id = "test_user"
    task_id = "test_task"

    with patch.object(cv_service, '_save_locally', new_callable=AsyncMock) as mock_save_locally:
      mock_save_locally.return_value = "2026/03/test_user/test_task.pdf"

      result = await cv_service.save_pdf(user_id, task_id, sample_html)

      mock_save_locally.assert_called_once_with(user_id, task_id, sample_html)
      assert result == "2026/03/test_user/test_task.pdf"

  @pytest.mark.asyncio
  async def test_save_pdf_returns_path_from_save_locally(self, cv_service, sample_html):
  ...
  @pytest.mark.asyncio
  async def test_save_pdf_passes_correct_arguments(self, cv_service, sample_html):
      ...
      with patch.object(cv_service, '_save_locally', new_callable=AsyncMock) as mock_save_locally:
        ...
        await cv_service.save_pdf(user_id, task_id, sample_html)
  ...
  async def test_get_locally_constructs_correct_file_path(self, cv_service):
    ...
    with patch('app.services.cv_service.aiofiles.open', new_callable=AsyncMock) as mock_open_func:
      ...
      expected_full_path = cv_service.local_storage_path / pdf_path
  ...
  async def test_get_pdf_delegates_to_get_locally(self, cv_service):
    ...
    with patch.object(cv_service, '_get_locally', new_callable=AsyncMock) as mock_get_locally:
      mock_get_locally.return_value = fake_pdf

      result = await cv_service.get_pdf(pdf_path)

      mock_get_locally.assert_called_once_with(pdf_path)
      assert result == fake_pdf

  @pytest.mark.asyncio
  async def test_get_pdf_returns_bytes(self, cv_service):
  ...
  @pytest.mark.asyncio
  async def test_get_pdf_passes_correct_path(self, cv_service):
    ...
    with patch.object(cv_service, '_get_locally', new_callable=AsyncMock) as mock_get_locally:
      ...
      await cv_service.get_pdf(pdf_path)
  ...
  async def test_full_cv_workflow(self, cv_service, sample_cv_data):
    ...
    with patch('app.services.cv_service.Environment') as mock_env_class:
      ...
      generated_html = cv_service.generate_cv_html(sample_cv_data)
      ...
      with patch('app.services.cv_service.HTML') as mock_html_class:
        ...
        pdf_path = await cv_service.save_pdf(user_id, task_id, generated_html)
